...
- Milvus（含 2.5.x）本身不负责文本向量化，不存在“配置 text-embedding 后自动生成向量”的流程。
- 本项目由应用侧显式调用 Spring AI `EmbeddingModel` 生成向量，再写入 Milvus 的 `vector` 字段。
- 若未生成向量或向量为空，写入会失败并可能触发 `got nil`。
