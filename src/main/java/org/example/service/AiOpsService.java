...
public class AiOpsService {
    ...
    @Autowired
    private DateTimeTools dateTimeTools;// 日期的工具类  通过调用时区
    ...
    public Optional<OverAllState> executeAiOpsAnalysis(DashScopeChatModel chatModel, ToolCallback[] toolCallbacks) throws GraphRunnerException {
        ...
        ReactAgent plannerAgent = buildPlannerAgent(chatModel, toolCallbacks); // 构造计划制定者角色也是报告的整理者  领导
        ReactAgent executorAgent = buildExecutorAgent(chatModel, toolCallbacks); // 构造执行者角色   员工

        // 构建 Supervisor Agent  supervisor->   主管
    ...
    public Optional<String> extractFinalReport(OverAllState state) {
        ...
        Optional<AssistantMessage> plannerFinalOutput = state.value("planner_plan")
                .filter(AssistantMessage.class::isInstance)  // 这里过滤掉其他的像userMessage\systemMessage\ToolMessage等
    ...
    private ReactAgent buildPlannerAgent(DashScopeChatModel chatModel, ToolCallback[] toolCallbacks) {
        return ReactAgent.builder()
                .name("planner_agent")
                .description("负责拆解告警、规划与再规划步骤") //react prompt
                .model(chatModel)
